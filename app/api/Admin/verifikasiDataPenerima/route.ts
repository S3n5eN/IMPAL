...
async function verifikasiDataPenerima(req: NextRequest, decoded: { id: string }) {
  try {
    ...
    if (!isPending) {
        return NextResponse.json({message: "Shipment tidak ditemukan atau bukan dalam status pending"}, {status: 404})
    }
    ...
    if (action === "approve") {
        ...
        return NextResponse.json({message: "Data penerima berhasil diverifikasi"}, {status: 200})
    }
    ...
    if (action === "reject") {
        ...
        return NextResponse.json({message: "Data penerima ditolak"}, {status: 200})
    }
  } catch (error) {
